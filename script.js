...
function sendEmail(e) {
  ...
  function scrollToHashWithOffset() {
    ...
    // Recompute in case the nav height changed with breakpoint
    const nav = document.querySelector('nav');
    const offset = nav ? nav.offsetHeight : 0;

    // Wait a tick for layout (fonts/images) to settle, then scroll precisely
    // This double rAF pattern is a reliable, lightweight "after layout" hook.
  ...
  window.addEventListener('pageshow', (e) => {
    if (e.persisted) { // coming from back/forward cache
      applyScrollPadding();
      scrollToHashWithOffset();
    }
  });
